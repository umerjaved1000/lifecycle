$(document).ready(function () {
    ...
    if (jQuery('.schedule_form').length) {
...
$('#rootwizard .finish').click(function () {
    var $validator = $('#schedule_form').data('bootstrapValidator').validate();
    if ($validator.isValid()) {
        document.getElementById("schedule_form").submit();
    }
});
}
    if (jQuery('#schedule_calendar').length) {
        bookingCalendar = jQuery('#schedule_calendar').fullCalendar({
...
//        timezone: 'UTC'
    ...
    function show_calender_event_details_overview(data) {
        jQuery.ajax({
            type: "POST",
            url: '/schedule/calendar_event_details',
            data: {data: data},
            success: function (response) {
